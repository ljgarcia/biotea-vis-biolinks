...
gulp.task('test-env', ['init-test-reports'], function() {
    env({
        vars: {
            XUNIT_FILE: 'reports/TEST-BiolinksViewerTest.xml',
            LOG_XUNIT: true
        }
    });
});
...
gulp.task('copy-resources', ['init'], function() {
        ...
        return gulp.src(["./style/*.css", "./node_modules/biotea-vis-tooltip/style/bioteaTooltip.css",
            "node_modules/biotea-vis-annotation/style/bioteaAnnotation.css",
            "node_modules/biotea-vis-similarity/style/bioteaSimilarity.css",
            "node_modules/biotea-vis-topicDistribution/style/bioteaTopicDist.css"])
                .pipe(minifyCss({compatibility: 'ie8'}))
                .pipe(concat('bioteaBiolinks.css'))
                .pipe(gulp.dest(buildDir));
});
