...
var initSimilarityViewers = function(viewer) {
...
var initAnnotations = function(viewer) {
    ...
    viewer.annotSectionTitle = viewer.annotationGroupDiv.append('h2')
        .text('Semantic annotations for selected reference and compared articles');
...
var initAnnotationViewers = function(viewer) {
    ...
    addVisualRows(viewer, viewer.annotationReferenceTable, 'Annotations (reference article) - ', 'biolinks_annot_ref_',
        'biolinks_annotation_ref_viewer biolinks_vis_annotations' , viewer.annotationViewersReference);

    viewer.annotationViewersCompared = {};
    viewer.annotationComparedTable = viewer.annotationGroupDiv.append('table')
        .attr('width', '100%').style('display', 'none');
    addVisualRows(viewer, viewer.annotationComparedTable, 'Annotations (compared article) - ', 'biolinks_annot_comp_',
        'biolinks_annotation_comp_viewer biolinks_vis_annotations', viewer.annotationViewersCompared);
};
        ...
        .then(function() {
            ...
            if (viewer.articleTitle !== undefined) {
                if (viewer.topicArticles.length >= 3) {
                    ...
                    viewer.annotSectionTitle
                        .text('Semantic annotations for selected reference article');
    ...
    _.each(viewer.similarityViewers, function(simViewer, key) {
        ...
        viewer.similarityViewers[key].viewer.getDispatcher().on('selected', function(obj) {
            if (obj.type === 'similarity') {
                ...
                //TODO change if we keep seeAlso
                var comparedText;
                if (viewer.selectedContent.url) {
                ...
                if (viewer.selectedReferenceArticle.id !== viewer.selectedComparedArticle.id) {
                    ...
                    updateAnnotations(viewer, viewer.annotationViewersCompared, 'biolinks_annotation_comp_viewer',
                        viewer.selectedComparedArticle, obj.datum);
                } else {
                    d3.selectAll('.biolinks_annotation_comp_viewer').selectAll('*').remove();
                    d3.selectAll('.biolinks_annotation_comp_viewer').html('');
                    viewer.annotationComparedTable.style('display', 'none');
...
var updateAnnotations = function(viewer, annotViewers, className, selectedArticle, similarity) {
    d3.selectAll('.' + className).selectAll('*').remove();
    d3.selectAll('.' + className).html('');

    var termIds = [], termAltIds = [];
    if (similarity) {
        if (similarity.relatedId === selectedArticle.id) {
            termIds = similarity.terms;
            if (viewer.selectedContent.seeAlso) {
                var alt = _.find(viewer.similarityViewers[viewer.selectedContent.seeAlso].viewer.data.relations,
                    function (relation) {
                        return relation.relatedId === selectedArticle.altId;
                    }
                );
                termAltIds = alt ? alt.terms : [];
            }
        } else {
            var init = _.find(viewer.similarityViewers[viewer.selectedContent.label].viewer.data.relations,
                function (relation) {
                    return relatedId.relatedId === selectedArticle.id;
                }
            );
            termIds = init ? init.terms : [];
            termAltIds = similarity.terms;
        }
    }
    console.log(viewer.selectedContent);
    console.log(selectedArticle);
console.log(annotViewers);
    var keys = _.keys(annotViewers);
    _.each(annotViewers, function(annotViewer, key) {
        if (key === viewer.selectedContent.label) {
            annotViewer.viewer = createAnnotations(viewer, key, keys.length, annotViewer, selectedArticle.id, termIds);
...
var createAnnotations = function(viewer, key, length, annotViewer, id, terms) {
